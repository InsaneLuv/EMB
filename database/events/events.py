...
async def get_rabbit_top(game_name):
    rank = {
        'rank': 0,
        'count': 0
    }
    try:
        cur.execute(
            "SELECT game_name, count, (SELECT COUNT(*)+1 FROM rabbit_top AS t2 WHERE t2.count > t1.count) AS rank FROM rabbit_top AS t1 WHERE game_name = ?",
            (game_name,))
        result = cur.fetchone()
        rank = {
            'rank': result[2],
